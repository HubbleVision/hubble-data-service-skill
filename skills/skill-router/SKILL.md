...
**唯一授权数据源：Hubble 私有数据服务（V2 API）**

| 场景 | 正确做法 | 错误做法 |
|------|----------|----------|
| 查股价 | `curl $BASE/api/v2/.../securities` | WebSearch 搜"茅台股价" |
| 查财报 | `curl $BASE/api/v2/.../finance/income` | WebFetch 抓东方财富 |
| 查指标 | `curl $BASE/api/v2/indicators/batch` | 从记忆中凭空报数 |
| API 无此数据 | 告知用户"该数据暂不在覆盖范围内" | 转而去网上搜索 |

**如果你发现自己在尝试使用 WebSearch 或 WebFetch 获取金融数据，立即停止，改为调用 Hubble API。**
